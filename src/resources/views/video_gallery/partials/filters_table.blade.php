    ...
    <tr class="image-storage-filters-row">
        ...
        <td>
            <select name="image_storage_filter[filterByTags][]" multiple="multiple" class="image-storage-select">
                @foreach($relatedEntities['tag'] as $tag)
                    <option value="{{$tag->id}}"
                           {{in_array($tag->id,Session::get('image_storage_filter.video_gallery.filterByTags', array())) ? "selected" : ""}}>
                        {{$tag->title}}
                    </option>
                @endforeach
            </select>
        ...
        <td>
            <select name="image_storage_filter[filterByActivity][]" multiple="multiple" class="image-storage-select">
                    <option value="1"
                            {{in_array(1,Session::get('image_storage_filter.video_gallery.filterByActivity', array())) ? "selected" : ""}}>
                    {{__cms('Активна') }}</option>
                    <option value="0"
                            {{in_array(0,Session::get('image_storage_filter.video_gallery.filterByActivity', array())) ? "selected" : ""}}>
